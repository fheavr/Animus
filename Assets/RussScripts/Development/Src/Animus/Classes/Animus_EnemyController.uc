/* (c) Russell Gillette 2013
 */
 
class Animus_EnemyController extends UDKBot
...
{
    return LineOfSightTo(P) && Sight_Range >= VSize2D(Pawn.Location - P.Location);
}
...
function Animus_Pawn Detected()
{
    ...
    // check every pawn within the given detect range
    foreach WorldInfo.AllPawns(class'Animus_Pawn', P, Pawn.Location, Detect_Range)
	{
        if (P != None)   // if there is a pawn
        {
            // check sight and distance from pawn vs detection ranges and that
            // the pawn is the player (or an ally of the player)
            if (Sight(P) && (P.bIsPlayer || P.bIsAlly))
            {
